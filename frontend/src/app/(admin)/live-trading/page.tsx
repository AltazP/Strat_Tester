...
const getWebSocketUrl = () => {
  ...
  if (backendUrl.startsWith('https://')) {
    return backendUrl.replace("https://", "wss://") + "/live-trading/ws/sessions";
  } else {
    return backendUrl.replace("http://", "ws://") + "/live-trading/ws/sessions";
  }
};
...
function LiveTradingPageOld() {
  ...
  async function loadData() {
    ...
    try {
      const [accountsRes, sessionsRes, strategiesRes] = await Promise.all([
        fetch(`${BE}/live-trading/accounts`),
        fetch(`${BE}/live-trading/sessions`),
  ...
  async function loadAccounts() {
    try {
      const accountsRes = await fetch(`${BE}/live-trading/accounts`, {
        cache: 'no-store',
        headers: {
  ...
  async function fetchPositions(sessionId: string) {
    try {
      const res = await fetch(`${BE}/live-trading/sessions/${sessionId}/positions`);
      if (res.status === 404) {
        setPositions(prev => {
          const next = { ...prev };
  ...
  async function fetchTrades(sessionId: string) {
    try {
      const res = await fetch(`${BE}/live-trading/sessions/${sessionId}/trades`);
      if (res.status === 404) {
        setTrades(prev => {
          const next = { ...prev };
          delete next[sessionId];
  ...
  async function createSession(data: Record<string, unknown>) {
    try {
      const res = await fetch(`${BE}/live-trading/sessions`, {
        method: "POST",
  ...
  async function startSession(sessionId: string) {
    try {
      const res = await fetch(`${BE}/live-trading/sessions/${sessionId}/start`, { 
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
  ...
  async function stopSession(sessionId: string) {
    try {
      const res = await fetch(`${BE}/live-trading/sessions/${sessionId}/stop`, { method: "POST" });
  ...
  async function pauseSession(sessionId: string) {
    try {
      const res = await fetch(`${BE}/live-trading/sessions/${sessionId}/pause`, { method: "POST" });
  ...
  async function resumeSession(sessionId: string) {
    try {
      const res = await fetch(`${BE}/live-trading/sessions/${sessionId}/resume`, { method: "POST" });
  ...
  async function deleteSession(sessionId: string) {
    ...
    try {
      const res = await fetch(`${BE}/live-trading/sessions/${sessionId}`, { method: "DELETE" });
  ...
  return (
    <div className="space-y-6">
      <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Live trading with real OANDA accounts
          </p>
      </div>
      ...
      <div>
        ...
        {sortedAccounts.length === 0 ? (
          <div className="rounded-xl border-2 border-dashed border-stroke dark:border-strokedark bg-white dark:bg-gray-900 p-12 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Please check your OANDA_LIVE_API_KEY in the backend .env file</p>
          </div>
        ) : (
